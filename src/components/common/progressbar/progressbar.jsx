    ...
    <div className={'row progress-container '+ classNameList.join(' ')} data-align={configurations.titleAlignment}>
      ...
      <div className={configurations.titleAlignment === 'row'? 'col-8': 'col-12'}>
        <div className='progress-wrapper'>
          ...
          <div className="time-wrapper c-font-14">
            <span data-state={(time.week > 0) ? 'active' : 'mute'}>{time.week}W : </span>
            <span data-state={(time.day > 0) ? 'active' : 'mute'}>{time.day}D : </span>
            <span data-state={(time.hour > 0) ? 'active' : 'mute'}>{time.hour}h : </span>
            <span data-state={(time.minute > 0) ? 'active' : 'mute'}>{time.minute}m</span>
          </div>
        </div>
      </div>      
    </div>
