...
function Incident(props) {
  ...
  const renderTabBody = (tabName) => {
    switch (tabName) {
      case "incident_details":
        return <Details className="h-100" messageList={props.messageList} />;
      case "executive_summary":
        return <Summary className="h-100" />;
      default:
        break;
    }
  };
  ...
  return (
    <section className="section-incident p-0">
      <ul
        className="nav nav-tabs nav-fill mb-2"
        id="incident-tab"
        role="tablist"
      >
        {tabNames.map((tabData, tabIndex) => (
          <li
            key={`tab-${tabData.id}-${tabIndex}`}
            className="nav-item"
            role="presentation"
          >
            <button
